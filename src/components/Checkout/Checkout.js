...
const CheckOut = () => {
  ...
  useEffect(() => {
    fetch(
      `https://painting-path-solution-server.vercel.app/singleService/${id}`
    )
      .then((res) => res.json())
  ...
  const handleCheckOut = (product) => {
    ...
    fetch(`https://painting-path-solution-server.vercel.app/addOrder`, {
      method: "POST",
      headers: {
        ...
        <div className="col-md-7">
          <div className="payment-system pt-5">
            <div className="row">
              <div className="col">
                <div className="img">
                  <img className="img-fluid w-100" src={stripe} alt="" />
                </div>
              </div>
            </div>
          </div>
