...
const OrderList = () => {
  const pendingOrder = (id) => {
    ...
    fetch(
      `https://painting-path-solution-server.vercel.app/updateStatus/${id}`,
      {
        method: "PATCH",
  ...
  useEffect(() => {
    fetch(`https://painting-path-solution-server.vercel.app/ordersList`)
      .then((res) => res.json())
                ...
                {orders &&
                  orders.map((order) => (
                    <tr key={order._id}>
                      ...
                      <td>
                        <button
                          onClick={() => pendingOrder(order._id)}
                        ...
                        <button
                          onClick={() => onGoingOrder(order._id)}
